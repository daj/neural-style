...
Use the `--checkpoint-output` and `--checkpoint-iterations` to save checkpoint images.

Use the `--iterations` parameter to change the number of iterations.  The default number of iterations is 1000, which takes around 10 hours to run on a Mid 2013 Macbook Air for a 528×960 pixel content image.  
